...
function createFieldKey(campus: Campus, compactDate: string, mealCode: "a" | "b" | "c") {
...
export async function getMenuByDate(
  ...
  const [year, month] = compactDate.match(/^(\d{4})(\d{2})\d{2}$/)?.slice(1) ?? [];

  if (!year || !month) {
    throw new Error(`잘못된 날짜 형식입니다: ${dateKey}`);
  }

  const menuRef = doc(getDb(), "menu", campus, `year_${year}`, `month_${month}`);
